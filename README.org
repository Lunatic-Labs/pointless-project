...
**** Naming format:
- test- =FeatureBeingTested= - =webpage= .sh
**** Testing format:
#+begin_src bash
  #!/bin/bash
...
- Merge all HTML files with corresponding php
  - This will help with testing
- Clean up HTML code
  - Unclosed tags
  - Dedicated CSS for all web pages
- Functioning download button in download.php
  - Goal: user retrieves 7zip file from webpage via php
- Login page for registered users
- Default level zero for new users
  - Currently, index.php only records first and last name and email
  - User is to increase level everytime a token found in the puzzle is submitted
