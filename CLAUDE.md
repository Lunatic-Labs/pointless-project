...
- `admin.php` is the staff report (`includes/admin.php`): every player's status (finished, struggling, stalled, not started, active), progress, and events, with a CSV per tab. It is the only page with passwords: admins are `name:bcrypt` lines in `admins.htpasswd` next to the players file (`POINTLESS_ADMINS_FILE`); without that file it is a 404. Failed sign-ins lock sign-in globally (`admin-log.csv`). Unlike player pages, it may show counts.
