...
- `index.php` registers a player and `login.php` checks registration, both through `includes/players.php`. Emails are normalized (trimmed, lowercased) before validation, storage, and lookup; names are stored as typed and escaped only on output. Players are stored in `data/contact-data.csv` at the repository root (outside the web root and ignored by git), with columns `FName,LName,Email`. Both pages regenerate the session id and store the email in `$_SESSION["email"]`.
