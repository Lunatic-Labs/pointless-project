...
1. **"0 points"** — awarded on every page, and [files-math/.desc.txt](puzzle-code/resources/files-math/.desc.txt) sets it up with "Make sure to keep track of all the points you earn." Payoff in [files-fin](puzzle-code/resources/files-fin/.desc.txt): "Points earned: 0."
...
- **The map.** [files-bst/.desc.txt](puzzle-code/resources/files-bst/.desc.txt) already says *"you lost your map!"* — with no map ever having been given. Award a (wrong) map at maze, and that line becomes a payoff instead of a non-sequitur.
- **The battery.** [files-binary-addition](puzzle-code/resources/files-binary-addition/.desc.txt) opens with the robot *"shut down, its energy reserves depleted."* Award a dead battery there and the joke writes itself.

**Implementation:** hand-write a growing `<div class="callout">` inventory into each `.desc.txt`, right under the opening line. Each page already hard-codes its predecessor's name, so hard-coding the list is consistent with how the pages work and needs no C++ change. (The alternative — accumulating a `strvec_t` in `game_create_puzzles` and having `utils_html_printf` inject an inventory block next to the token block, the way [utils.cpp:207-212](puzzle-code/src/utils.cpp#L207-L212) does — is cleaner but touches the signature on all 14 call sites. Only worth it if you expect to reorder puzzles.)
...
"It May Help You on Your Quest" works best as a *character tic*. You already have an Oracle in [files-rematch-maze](puzzle-code/resources/files-rematch-maze/.desc.txt); promote it to the recurring Old Person Who Hands You Things, and let the line repeat verbatim on every award.
...
Keep the substring `pointless journey` intact — [fin-puzzle-test.cpp:7](puzzle-code/tests/fin-puzzle-test.cpp#L7) asserts it (and also asserts the whole `.desc.txt` is embedded, so the rest of that file is self-updating).

## Page titles: free trope flavor

The `<h2>` on each page comes from the first argument to `utils_html_printf`, and they're currently utilitarian — "Color Puzzle", "Encrypt", "BST Puzzle", "Base Intro Puzzle". These are pure string literals in `.cpp`, affect no rolls, and appear in no test. Cheapest high-impact edit in the whole review:

| File | Current | Suggested |
|---|---|---|
| [math-puzzle.cpp:25](puzzle-code/src/math-puzzle.cpp#L25) | Hello Pointless | The Call to Adventure |
| [color-puzzle.cpp:21](puzzle-code/src/color-puzzle.cpp#L21) | Color Puzzle | The Ordinary World, in Hexadecimal |
| [pixel-puzzle.cpp:49](puzzle-code/src/pixel-puzzle.cpp#L49) | Pixel Puzzle | Trust Nothing, Especially Lou |
| [maze-puzzle.cpp:81](puzzle-code/src/maze-puzzle.cpp#L81) | Maze Puzzle | The Labyrinth |
| [based-intro-puzzle.cpp:58](puzzle-code/src/based-intro-puzzle.cpp#L58) | Base Intro Puzzle | The Cryptic Artifact |
| [encrypt-puzzle.cpp:124](puzzle-code/src/encrypt-puzzle.cpp#L124) | Encrypt | The Eccentric Genius |
| [rematch-puzzle.cpp:52](puzzle-code/src/rematch-puzzle.cpp#L52) | Rematch Instructions | Gather the Plot Coupons |
| [binary-addition-puzzle.cpp:66](puzzle-code/src/binary-addition-puzzle.cpp#L66) | Graph Paper Robot | The Companion Reawakens |
| [logicgate-puzzle.cpp:87](puzzle-code/src/logicgate-puzzle.cpp#L87) | Graph Paper Robot PT II | The Tower of Wisdom |
| [bst-puzzle.cpp:161](puzzle-code/src/bst-puzzle.cpp#L161) | BST Puzzle | Lost Without a Map |
| [fin-puzzle.cpp:8](puzzle-code/src/fin-puzzle.cpp#L8) | Goodbye Pointless | The MacGuffin Revealed |
...
- **The golden robot** → the Loyal Sidekick. Note that [logicgate](puzzle-code/resources/files-logicgate/.desc.txt) is titled "Graph Paper Robot PT II" but its text never mentions the robot at all — that's a free place to bring it back.
- **Dr. Pepper** → the Mad Scientist. [files-rematch-encrypt](puzzle-code/resources/files-rematch-encrypt/.desc.txt) already has his slapstick voice ("we *accidentally* dropped the password into a goopifying machine") without attributing it to him. Attribute it.
- **Lou** → the Unreliable Narrator, already perfect in one line: *"Lou seems a little off today… or maybe it's you."*

## Continuity nits

- **"alien lightbox"** ([files-encrypt](puzzle-code/resources/files-encrypt/.desc.txt)) refers back to based-intro, whose light box isn't alien — it's "a cryptic structure." The *alien* one is [files-rematch-based](puzzle-code/resources/files-rematch-based/.desc.txt), which comes later. Either make based-intro's light box explicitly alien, or change the callback. Note the string is asserted at [encrypt-puzzle-test.cpp:12](puzzle-code/tests/encrypt-puzzle-test.cpp#L12), so changing it means updating that line.
- **"You seemed to struggle on the first encryption puzzle"** ([files-rematch-encrypt](puzzle-code/resources/files-rematch-encrypt/.desc.txt)) presumes a failure that may not have happened. Trope-ify it instead: *"The Oracle has reviewed your performance and is concerned. It would like to give you a freebie."*
